...
if selected == "Prediksi Pertandingan":
    ...
    if historical_data_file is not None and fixture_file is not None:
        ...
        def calculate_winner(country1_name, country2_name, powerlevel):
            country1_goals_scored_lambda = powerlevel.loc[country1_name, 'GoalsScored']
            country1_goals_conceded_lambda = powerlevel.loc[country1_name, 'GoalsConceded']
    
            country2_goals_scored_lambda = powerlevel.loc[country2_name, 'GoalsScored']
            country2_goals_conceded_lambda = powerlevel.loc[country2_name, 'GoalsConceded']
    
            prob_home_win = sum(poisson.pmf(i, country1_goals_scored_lambda) * poisson.pmf(j, country2_goals_conceded_lambda) 
                              for i in range(7) for j in range(7) if i > j)
            prob_away_win = sum(poisson.pmf(i, country2_goals_scored_lambda) * poisson.pmf(j, country1_goals_conceded_lambda) 
                                for i in range(7) for j in range(7) if j > i)
